...
## Compare Tutor and Student Schedules
The following R scripts query to find a tutor with a preference that matches the level of the target student, then compares student and tutor schedules to find which tutors are working when the student is available.
...
query_tutors <- glue("
  ...
  FROM Tutors
  JOIN Preferences ON Tutors.Tutor_ID = Preferences.Tutor_ID
  JOIN Students ON Students.Name = '{name}'
  WHERE 
      CASE 
          WHEN Students.Level = 'ESL' THEN Preferences.ESL
          WHEN Students.Level = 'Conversation' THEN Preferences.Conversation
...
tutors <- dbGetQuery(dbcon, query_tutors)

print(tutors)

for (i in 1:nrow(tutors)) {
  tutor_name <- tutors$Name[i]
  cat("Tutor:", tutor_name, "\n")
}
```
